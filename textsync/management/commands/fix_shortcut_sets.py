...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        unlinked_shortcuts = Shortcut.objects.annotate(set_count=Count("sets")).filter(
            set_count=0
        )
        ...
        if unlinked_count == 0:
            self.stdout.write(
                self.style.SUCCESS("✅ All shortcuts are already linked to sets!")
            )
            return

        self.stdout.write(
            self.style.WARNING(
                f"⚠️  Found {unlinked_count} shortcuts not linked to any sets"
            )
        )
        ...
        try:
            ...
            self.stdout.write(
                f"📁 Target set: '{target_set.name}' ({target_set.get_set_type_display()})\n"
            )
        ...
        for sc in unlinked_shortcuts[:5]:
            ...
            preview = (
                sc.value[:40]
                if sc.value
                else (sc.html_value[:40] if sc.html_value else "")
            )
        ...
        if dry_run:
            ...
            self.stdout.write(
                f"   Would link {unlinked_count} shortcuts to '{target_set.name}' set"
            )
            return
        ...
        self.stdout.write(
            f"   Total shortcuts in {target_set.name}: {target_set.shortcuts.count()}"
        )
